...
public class MatrixBombItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.BLOCK_GLASS_HIT, SoundCategory.NEUTRAL, 0.5F, 2 / (world.random.nextFloat() * 0.4F + 0.8F));

        if (!world.isClient) {
            MatrixBombEntity proj = new MatrixBombEntity(OmegaEntities.MATRIX_BOMB, world);
            proj.setPos(user.getX(), user.getEyeY() - .5, user.getZ());
            proj.setOwner(user);
            proj.setVelocity(user, user.getPitch(), user.getYaw(), 0, 1, 1);
