...
public class NovaZoneStructure extends StructureFeature<DefaultFeatureConfig> {

    private static final SimplexNoiseSampler NOISE = new SimplexNoiseSampler(new SimpleRandom(0));

    public NovaZoneStructure(Codec<DefaultFeatureConfig> configCodec) {
        super(configCodec, StructureGeneratorFactory.simple(StructureGeneratorFactory.checkForBiomeOnTop(Heightmap.Type.WORLD_SURFACE_WG), NovaZoneStructure::addPieces));
    }
    ...
    public static void addPieces(StructurePiecesCollector collector, StructurePiecesGenerator.Context<DefaultFeatureConfig> context) {
        ...
        collector.addPiece(new VoidPiece(blockPos));
        collector.addPiece(new BasePiece(blockPos));
    }

    public static class VoidPiece extends StructurePiece {

        public VoidPiece(BlockPos pos) {
            super(OmegaStructurePieces.NOVA_ZONE_VOID, 0,
                    new BlockBox(pos.getX() - 128, pos.getY() - 64, pos.getZ() - 128, pos.getX() + 128, pos.getY() + 250, pos.getZ() + 128));

            setOrientation(null);
        }

        public VoidPiece(StructureContext context, NbtCompound compound) {
            super(OmegaStructurePieces.NOVA_ZONE_VOID, compound);
            setOrientation(null);
        }
    ...
    public static class BasePiece extends StructurePiece {
        ...
        public BasePiece(BlockPos pos) {
            super(OmegaStructurePieces.NOVA_ZONE_BASE, 0, new BlockBox(pos.getX() - 128, pos.getY() - 48, pos.getZ() - 128, pos.getX() + 128, pos.getY() + 250, pos.getZ() + 128));
            setOrientation(null);
        }

        public BasePiece(StructureContext context, NbtCompound compound) {
            super(OmegaStructurePieces.NOVA_ZONE_BASE, compound);
            setOrientation(null);
        }
        ...
        public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pos) {
            ...
            cache.placeOrCompute(1, world, this, chunkBox, chunkPos, (blocks) -> {
                new NovaZoneGenerator(this, chunkBox, blocks).spawn(world, pos);
                return new StructureCache.ChunkSectionedEntry(blocks);
            });
        }
    }
