...
public class EyeAltarGenerator extends SimpleStructurePiece {

    private static final Identifier TEMPLATE = IntoTheOmega.id("eye_altar");
    private static final Identifier LOOT_TABLE = IntoTheOmega.id("chests/eye_altar");

    public EyeAltarGenerator(StructureManager manager, BlockPos pos, BlockRotation rotation) {
        super(OmegaStructurePieces.EYE_ALTAR, 0, manager, TEMPLATE, TEMPLATE.toString(),  new StructurePlacementData().setRotation(rotation), pos);
    }

    public EyeAltarGenerator(StructureContext context, NbtCompound nbt) {
        super(OmegaStructurePieces.EYE_ALTAR, nbt, context.structureManager(), id -> new StructurePlacementData().setRotation(BlockRotation.valueOf(nbt.getString("Rotation"))));
    }
