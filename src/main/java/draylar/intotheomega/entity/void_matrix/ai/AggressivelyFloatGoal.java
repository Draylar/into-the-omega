...
public class AggressivelyFloatGoal extends StageGoal {
    ...
    public AggressivelyFloatGoal(VoidMatrixEntity entity, VoidMatrixEntity.Stage stage) {
        super(entity, stage);
    ...
    public void start() {
        ...
        if(target != null) {
            double destinationX = target.getX() + (double) ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
            double destinationY = target.getY() + (double) ((random.nextFloat()) * 8.0F + 2);
            double destinationZ = target.getZ() + (double) ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);

            this.vm.getMoveControl().moveTo(destinationX, destinationY, destinationZ, 1.0D);
        }
    }
}
