...
public class SwirledMixerRenderer extends BlockEntityRenderer<SwirledMixerBlockEntity> {

    public SwirledMixerRenderer(BlockEntityRenderDispatcher dispatcher) {
        super(dispatcher);
    }
    ...
    public void render(SwirledMixerBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if(!catalyst.isEmpty()) {
            ...
            matrices.multiply(Vector3f.POSITIVE_Y.getDegreesQuaternion(lerpedAge * 2));
            renderer.renderItem(catalyst, ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumers);
            matrices.pop();
        }
        ...
        for (ItemStack potion : entity.getPotions()) {
            if(!potion.isEmpty()) {
                ...
                matrices.scale(0.35f, 0.35f, 0.35f);
                matrices.multiply(Vector3f.POSITIVE_Y.getDegreesQuaternion(lerpedAge * 5));
                renderer.renderItem(potion, ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumers);
                matrices.pop();
            }
        }
    }
